...
func (h *Service) CreateBrokerImage(ctx context.Context, req *protogen.CreateBrokerImageRequest) (*protogen.CreateBrokerImageResponse, error) {
	...
	if err != nil {
		...
		return &protogen.CreateBrokerImageResponse{}, status.Error(codes.InvalidArgument, "Invalid broker ID")
	}
	...
	if ok, err := brokerImageInput.IsValid(); !ok {
		...
		return &protogen.CreateBrokerImageResponse{}, status.Error(codes.InvalidArgument, err.Error())
	}
	...
	if err != nil {
		...
		return &protogen.CreateBrokerImageResponse{}, status.Error(codes.Internal, err.Error())
	}
	if ok {
		zap.L().Warn("Broker already has an image", zap.String("broker_id", brokerID.String()))
		return &protogen.CreateBrokerImageResponse{}, status.Error(codes.InvalidArgument, "Broker already has an image")
	}
	...
	if !found {
		...
		return &protogen.CreateBrokerImageResponse{}, status.Error(codes.Internal, "Broker image not found after create")
	}
...
func (h *Service) GetBrokerImage(ctx context.Context, req *protogen.GetBrokerImageRequest) (*protogen.GetBrokerImageResponse, error) {
	...
	if err != nil {
		...
		return &protogen.GetBrokerImageResponse{}, status.Error(codes.InvalidArgument, "Invalid image ID")
	}
	...
	if err != nil {
		...
		return &protogen.GetBrokerImageResponse{}, status.Error(codes.Internal, err.Error())
	}
	if !found {
		zap.L().Warn("BrokerImage does not exist", zap.String("image_id", imageID.String()))
		return &protogen.GetBrokerImageResponse{}, status.Error(codes.NotFound, "BrokerImage does not exist")
	}

	return &protogen.GetBrokerImageResponse{
...
func (h *Service) UpdateBrokerImage(ctx context.Context, req *protogen.UpdateBrokerImageRequest) (*protogen.UpdateBrokerImageResponse, error) {
	...
	if err != nil {
		// Log the error and return an invalid response
		zap.L().Error("Invalid image ID", zap.String("image_id", req.GetImageId()), zap.Error(err))
		return &protogen.UpdateBrokerImageResponse{}, status.Error(codes.InvalidArgument, "Invalid image ID")
	}
	...
	if err != nil {
		// Log the error and return an invalid response
		zap.L().Error("Invalid broker ID", zap.String("broker_id", req.GetBrokerId()), zap.Error(err))
		return &protogen.UpdateBrokerImageResponse{}, status.Error(codes.InvalidArgument, "Invalid broker ID")
	}
	...
	if ok, err := brokerImageInput.IsValid(); !ok {
		zap.L().Warn("BrokerImage is not valid", zap.Error(err))
		return &protogen.UpdateBrokerImageResponse{}, status.Error(codes.InvalidArgument, err.Error())
	}
	...
	if err != nil {
		...
		return &protogen.UpdateBrokerImageResponse{}, status.Error(codes.Internal, err.Error())
	}
	if !exists {
		zap.L().Warn("ImageBroker not found", zap.String("broker_id", brokerID.String()), zap.String("image_id", imageID.String()))
		return &protogen.UpdateBrokerImageResponse{}, status.Error(codes.NotFound, "ImageBroker not found")
	}
	...
	if !found {
		zap.L().Error("Broker image not found after create", zap.String("uuid", brokerImageInput.ID.String()))
		return &protogen.UpdateBrokerImageResponse{}, status.Error(codes.Internal, "Broker image not found after create")
	}
...
func (h *Service) DeleteBrokerImage(ctx context.Context, req *protogen.DeleteBrokerImageRequest) (*protogen.DeleteBrokerImageResponse, error) {
	...
	if err != nil {
		// Log the error and return an invalid response
		zap.L().Error("Invalid image ID", zap.String("image_id", req.GetImageId()), zap.Error(err))
		return &protogen.DeleteBrokerImageResponse{
			Success: false,
